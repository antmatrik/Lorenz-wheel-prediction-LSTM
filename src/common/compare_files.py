#!/usr/bin/env python3
"""Compare prediction CSVs against actual CSVs and write squared-error metrics.

Matches files by name across one actual directory and one or more model
prediction directories, then writes per-file and per-column MSE/SSE to a CSV.

Example:
    python src/common/compare_files.py --actual-dir data/actual \\
        --pred-dir lstm=outputs/lstm --pred-dir esn=outputs/esn \\
