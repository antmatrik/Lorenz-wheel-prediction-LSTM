#!/usr/bin/env python3
"""LSTM workflow with two modes:
1) train   -> train on many CSV files, save weights + normalization stats
2) predict -> load weights, use 5000 input points, export 1800 future predictions
"""
...
# -------- train mode settings --------
# Pattern that matches many training files (example: around 100 files).
TRAIN_FILES_GLOB = "csvs_noMdf/*.csv"

# How much of each file is used for training windows.
TRAIN_RATIO = 1

# Where to save learned model parameters and normalization stats.
WEIGHTS_OUTPUT_PATH = "lstm_weights.weights.h5"
STATS_OUTPUT_PATH = "lstm_stats.npz"
...
PREDICT_OUTPUT_PATH = "lstm_predictions.csv"

# Previously saved weights/stats paths to load.
WEIGHTS_INPUT_PATH = "lstm_weights.weights.h5"
STATS_INPUT_PATH = "lstm_stats.npz"
...
def plot_predictions(x_all: np.ndarray, cut: int, lstm_pred: np.ndarray):
    """Plot actual test region vs LSTM prediction."""
    t_test = x_all[cut:, 3]
    actual = x_all[cut:, :3]
    ...
    for i, ax in enumerate(axes):
        ...
        ax.plot(t_test, lstm_pred[:, i], label="LSTM", color="tab:blue", alpha=0.95)
    ...
    fig.suptitle("Lorenz Waterwheel: Actual vs LSTM")
    plt.tight_layout()
    plt.show()
